import './Option.css';

export default function Option() {
    return (
        <>
            <div className='option_outer'>
                
                <div className='option_row'>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>Process</label>
                        <label  className='option_row_col_small'>(ppt )</label>
                    </div>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>Current Lottery</label>
                        <label className='option_row_col_small'>A-20</label>
                    </div>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>You calculate your probable Prize wins</label>
                        <label className='option_row_col_small'>M3</label>
                    </div>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>Buy Lottery Unit</label>
                        <label className='option_row_col_small'>M1</label>
                    </div>
                </div>


                <div className='option_row'>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>View Tickets purchased</label>
                        <label className='option_row_col_small'>A17</label>
                    </div>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>View Info</label>
                        <label className='option_row_col_small'>A21</label>
                    </div>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>Community ...</label>
                        <label className='option_row_col_bold'>Charity</label>
                        <label className='option_row_col_small'>M3</label>
                        <label className='option_row_col_bold'>Surveys</label>
                        <label className='option_row_col_small'>M5</label>
                    </div>
                    <div className='option_row_col2'>
                        <label className='option_row_col_bold'>Kerala State Lottery Dept</label>
                    </div>
                </div>

            </div>
        </>
    )
};
